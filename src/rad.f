...
C        WRITE(*,'(A,I5,5F15.5)') 'J1,DIST,RADIUS in rad = ',J1,DIST,RADIUS,X(J3-2),X(J3-1),X(J3)
         IF (DIST.GT.RADIUS) THEN
            EVAP=.TRUE.
            if (evap.and.hist) then
                evapreject=.true.
                IF (DEBUG) PRINT*,'EVAP: atom, radius=',J1,SQRT(DIST)
                return
            endif
...
C        WRITE(*,'(A,I6,5F15.5)') 'J1,DIST,coords,RADIUS in radr=',J1,DIST,X(J3-2),X(J3-1),X(J3),RADIUS
         IF (DIST.GT.RADIUS) THEN
            EVAP=.TRUE.
            WRITE(*,'(A,I5,5G17.8)') 'EVAP: molecule, coords, dist, radius=',J1,X(J3-2),X(J3-1),X(J3),SQRT(DIST),SQRT(RADIUS)

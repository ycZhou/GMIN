...
C
      subroutine calc_gradientBLN(qo,fq,n,LJREP_BLN,LJATT_BLN,A_BLN,B_BLN,C_BLN,D_BLN,x,y,z,xr,yr,zr,dot_prod,x_prod,
     1  bond_angle,tor_angle,radii,natoms,RK_R,RK_THETA,COSTOR,DFAC,SINBOND)
      implicit NONE
      INTEGER NATOMS, N, I, J
...
C particles 1,2,n-1, and n done outside of the loop
      ...
      fba_x(i) = -rnum*((dot_prod(i,2)/dot_prod(i,1))*xr(i,i+1) - xr(i+1,i+2))/den

      fba_y(i) = -rnum*((dot_prod(i,2)/dot_prod(i,1))*yr(i,i+1) - yr(i+1,i+2))/den

      fba_z(i) = -rnum*((dot_prod(i,2)/dot_prod(i,1))*zr(i,i+1) - zr(i+1,i+2))/den
...
C particle 2
     ...
     1      *yr(i-1,i) + yr(i,i+1) - yr(i-1,i))/den
      ...
      a2 = -rk_theta*(bond_angle(i+1) - theta_0)*((dot_prod(i,2)/dot_prod(i,1))*yr(i,i+1) - yr(i+1,i+2))/den1
     ...
     1      *zr(i-1,i) + zr(i,i+1) - zr(i-1,i))/den
      ...
      a2 = -rk_theta*(bond_angle(i+1) - theta_0)*((dot_prod(i,2)/dot_prod(i,1))*zr(i,i+1) - zr(i+1,i+2))/den1

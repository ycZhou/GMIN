...
10    DO J1=1,NTARGETS
         IF (DABS(EREAL-TARGETS(J1)).LT.ECONV) THEN
            IF (NPAR.LT.2) THEN
               WRITE(*,'(2(A,I15),A)') 'Target hit after ',NQTOT,' quenches ',NPCALL,' function calls.'
               WRITE(*,'(2(A,F20.10))') 'Energy=',EREAL,' target=',TARGETS(J1)
            ELSE
               WRITE(*,'(A,I2,2(A,I15),A)') 'Target hit in parallel run ',NP,

...
C     WRITE(*,'(A,F20.10,A,F20.10)') 'Step length=',STRYDID,' next estimated step size=',STRYNEXT
      ...
      ELSE
         NBAD=NBAD+1
      ENDIF
C     PRINT*,'STRYDID,STRY,NOK,NBAD=',STRYDID,STRY,NOK,NBAD
      IF (ABS(STRYNEXT).LT.STRYMIN) THEN
         ...
         STOP
      ENDIF
      IF (DEBUG) WRITE(*,'(A,2F20.10,A,I6,A)') ' Energy and RMS force=',ENERGY,RMS,' after ',NSTP,' LBFGS steps'
...
C     PRINT*,'ERRMAX in rkqs=',ERRMAX
      IF (ERRMAX.LT.1.0D0) THEN
         CALL POTENTIAL(YTEMP,VNEW,ENERGY,.FALSE.,.FALSE.)
C        PRINT*,'ENERGY,EOLD=',ENERGY,EOLD
...
C     call derivs(x+A2*h,ytemp,ak2)
      ...
      CALL POTENTIAL(YTEMP,AK2,ENERGY,.TRUE.,.FALSE.)
...
C     call derivs(x+A3*h,ytemp,ak3)
      ...
      CALL POTENTIAL(YTEMP,AK3,ENERGY,.TRUE.,.FALSE.)
...
C     call derivs(x+A4*h,ytemp,ak4)
      ...
      CALL POTENTIAL(YTEMP,AK4,ENERGY,.TRUE.,.FALSE.)
...
C     call derivs(x+A5*h,ytemp,ak5)
      ...
      CALL POTENTIAL(YTEMP,AK5,ENERGY,.TRUE.,.FALSE.)
...
C     call derivs(x+A6*h,ytemp,ak6)
      ...
      CALL POTENTIAL(YTEMP,AK6,ENERGY,.TRUE.,.FALSE.)

...
C
      ...
      ENDDO
      IF (IDEGEN.EQ.0) THEN
         IF (IPRNT.GE.3) WRITE(*,'(A)') ' The molecule belongs to an Abelian group.'
         GOTO 630
      ENDIF
C
...
110      FORMAT(' The molecule belongs to a point group with doubly',
     1        ' degenerate representations.')
         ...
         IF (ILINEAR.EQ.1) THEN
            IF (IPRNT.GE.3) WRITE(*,'(A)') ' The molecule is linear.'
            GOTO 630
         ENDIF
C
...
C              IF (IPRNT.GE.4) WRITE(*,*) ICOMP
C              IF (ICOMP.EQ.0) THEN
               IF (IPRNT.GE.4) PRINT '(A,I5,3F12.3)','IAXORD,DUMMY,DIST2,TOLD=',I,DUMMY,DIST2,TOLD
...
CC                    PRINT*,'here A IGEN=',IGEN
                     IF (DEBUG.OR.(IPRNT.GE.3)) PRINT*,'axis ',J,' is a rotation axis of order ',I
C
C   Make the transformation matrix corresponding to the reference state in QREF
C
                     IF (DEBUG.OR.(IPRNT.GE.3)) PRINT *,'calling reorient A'
                     ...
                     IF (DEBUG.OR.(IPRNT.GE.3)) PRINT*,'here A IGEN=',IGEN,' new GENMAT:'
                     IF (DEBUG.OR.(IPRNT.GE.3)) PRINT '(9F12.4)',GENMAT(IGEN,1:3,1:3)
10                   CONTINUE
C                  ENDIF
C
...
C
         IF (IHIGH.EQ.0) THEN
            IF (DEBUG.OR.(IPRNT.GE.3)) PRINT*,'Accidental degeneracy detected'
            TOLE=TOLE/10.0D0
            IF (TOLE.GT.1.0D-7) THEN
               GOTO 651
            ELSE
               IF (DEBUG.OR.(IPRNT.GE.3)) PRINT*,' The full molecular point group is undetermined'
...
C              PRINT*,'I,IHIGH,NORD2=',I,IHIGH,NORD2(NATOMS+I)
...
255            IF (IREF.NE.0) THEN
                  IF (IPRNT.GE.13) WRITE(*,265) MINORB, NORD2(IREF)
...
256               IF (IREF.EQ.0) THEN
                     IF (DEBUG.OR.(IPRNT.GE.3)) PRINT '(A,G20.10)',
     &   'Orbit size inconsistent with axis order - try decreasing TOLD to ',TOLD/10.0D0
                     ...
                     ENDIF
                  ENDIF
                  IF (IPRNT.GE.13) WRITE(*,266) MINORB, NORD2(IREF)
...
C   Make the transformation matrix corresponding to the reference state in QREF 
C
               IF (DEBUG.OR.(IPRNT.GE.3)) PRINT *,'calling reorient F'
               CALL REORIENT(NATOMS,SCRATCH,QREF,RMAT) 
               CALL MYMATMUL(DUM,RM,RMAT,3,3,3)
               CALL MATMULV(DUM2,DUM,RMAT,3,3,3)
               DO J2=1,IGEN
               ...
               IGEN=IGEN+1
               GENMAT(IGEN,1:3,1:3)=DUM2(1:3,1:3)
               IF (DEBUG.OR.(IPRNT.GE.3)) PRINT*,'here F IGEN=',IGEN,' new GENMAT:'
               IF (DEBUG.OR.(IPRNT.GE.3)) PRINT '(9F12.4)',GENMAT(IGEN,1:3,1:3)
61          CONTINUE
            ENDIF
         ENDIF
...
360      CONTINUE
         IF (JCOMP.GE.1) THEN
            IF (IPRNT.GE.3) WRITE(*,'(A)') ' On-axis perpendicular C2 elements found.'
            IDGRP=1
         ELSEIF (JCOMPX.GE.1) THEN
            IF (IPRNT.GE.3) WRITE(*,'(A)') ' Off-axis perpendicular C2 elements found.'
            IDGRP=1
         ENDIF
C
C   Check for sigma(h)
...
C
      IF (IDEGEN.EQ.2) THEN
         IF (IPRNT.GE.3) WRITE(*,*) ' In cubic loop '
...
C
         DO 620 ICOUNT=1,2
            IF (IPRNT.GE.3) WRITE(*,*)' PASS ',ICOUNT,' THROUGH ROT. FINDER '
            DO 610 J=1,MINORB-1
               IF (IDONE.EQ.1) GOTO 610
               IF (IPRNT.GE.3) WRITE(*,*)' ATOMS ',NORD2(IREF),NORD2(IREF+J)
               IBOT2=3*NORD2(IREF+J)-2
               CALL XVEC(NEWQ(IBOT),NEWQ(IBOT2),SCRATCH(1),0)
               DIST=SQRT(MYDOT(SCRATCH(1),SCRATCH(1),3))
               CALL VADD(SCRATCH(1),NEWQ(IBOT),NEWQ(IBOT2),3*NATOMS,1)
C
...
C
C                 IF (IIAX.EQ.0) THEN
                  IF (IPRNT.GT.4) PRINT '(A,I5,3F12.3)','IAXORD,DUMMY,DIST2,TOLD=',IAXORD,DUMMY,DIST2,TOLD
C                 PRINT*,'QSORT:'
C                 PRINT '(3G20.10)',QSORT(1:3*NATOMS)
...
C   Make the transformation matrix corresponding to the reference state in QREF
C
                        IF (DEBUG.OR.(IPRNT.GE.3)) PRINT *,'calling reorient B'
                        ...
                        IGEN=IGEN+1
                        GENMAT(IGEN,1:3,1:3)=DUM2(1:3,1:3)
                        IF (DEBUG.OR.(IPRNT.GE.3)) PRINT*,'here B IGEN=',IGEN,' new GENMAT:'
                        IF (DEBUG.OR.(IPRNT.GE.3)) PRINT '(9F12.4)',GENMAT(IGEN,1:3,1:3)
20                      CONTINUE
                     ENDIF
                     IF (DEBUG.OR.(IPRNT.GE.3)) WRITE(*,580) IAXORD
580                  FORMAT(' Axis of order ',I1,' identified.')
                     IF (IAXORD.EQ.5) THEN
                        FPGRP='I  '
C                       IF (ISINV.EQ.0) FPGRP='I h'
                        IF (DUMMYSINV.LT.TOLD) FPGRP='I h'
                     ENDIF
                     IF (IAXORD.EQ.4) THEN
                        FPGRP='O  '
C                       IF (ISINV.EQ.0) FPGRP='O h'
                        IF (DUMMYSINV.LT.TOLD) FPGRP='O h'
C
...
C                          IF (ISINV.EQ.0) FPGRP='T h'
                           IF (DUMMYSINV.LT.TOLD) FPGRP='T h'
                        ENDIF
                     ENDIF
                     IF (IAXORD.EQ.2) THEN
...
C                                IF (ICOMPQ.EQ.0.AND.ISINV.NE.0) FPGRP='T d'
                                 IF ((DIST2.LT.TOLD).AND.(DUMMYSINV.GT.TOLD)) FPGRP='T d'
                                 CALL ZERO(NEWQ,3*NATOMS)
                                 CALL VADD(NEWQ,NEWQ,SCRATCH(3*NATOMS+1),3*NATOMS,1)
C                                CALL SORTXYZ(NEWQ,QSORT,NORD,TOLD,NATOMS)
                                 QSORT(1:3*NATOMS)=NEWQ(1:3*NATOMS)
...
C   Make the transformation matrix corresponding to the reference state in QREF 
C
               IF (DEBUG.OR.(IPRNT.GE.3)) PRINT *,'calling reorient C'
               CALL REORIENT(NATOMS,SCRATCH,QREF,RMAT) 
               CALL MYMATMUL(DUM,RM,RMAT,3,3,3)
               CALL MATMULV(DUM2,DUM,RMAT,3,3,3)
               DO J2=1,IGEN
               ...
               IGEN=IGEN+1
               GENMAT(IGEN,1:3,1:3)=DUM2(1:3,1:3)
               IF (DEBUG.OR.(IPRNT.GE.3)) PRINT*,'here C IGEN=',IGEN,' new GENMAT:'
               IF (DEBUG.OR.(IPRNT.GE.3)) PRINT '(9F12.4)',GENMAT(IGEN,1:3,1:3)
30             CONTINUE
            ENDIF
            IF (DEBUG.OR.(IPRNT.GE.3)) WRITE(*,640) I
...
C   Make the transformation matrix corresponding to the reference state in QREF 
C
               IF (DEBUG.OR.(IPRNT.GE.3)) PRINT *,'calling reorient D'
               CALL REORIENT(NATOMS,SCRATCH,QREF,RMAT) 
               CALL MYMATMUL(DUM,RM,RMAT,3,3,3)
               CALL MATMULV(DUM2,DUM,RMAT,3,3,3)
               DO J2=1,IGEN
               ...
               IGEN=IGEN+1
               GENMAT(IGEN,1:3,1:3)=DUM2(1:3,1:3)
               IF (DEBUG.OR.(IPRNT.GE.3)) PRINT*,'here D IGEN=',IGEN,' new GENMAT:'
               IF (DEBUG.OR.(IPRNT.GE.3)) PRINT '(9F12.4)',GENMAT(IGEN,1:3,1:3)
40             CONTINUE
            ENDIF
            IF (DEBUG.OR.(IPRNT.GE.3)) WRITE(*,650) I
...
C     IF ((ISAXIS.EQ.1).OR.(DIST2.LT.TOLD)) THEN
      IF (DIST2.LT.TOLD) THEN
         IF (DEBUG.OR.(IPRNT.GE.3)) WRITE(*,680)
...
C   Make the transformation matrix corresponding to the reference state in QREF 
C
            IF (DEBUG.OR.(IPRNT.GE.3)) PRINT *,'calling reorient E'
            CALL REORIENT(NATOMS,SCRATCH,QREF,RMAT) 
            CALL MYMATMUL(DUM,RM,RMAT,3,3,3)
            CALL MATMULV(DUM2,DUM,RMAT,3,3,3)
            DO J2=1,IGEN
            ...
            IGEN=IGEN+1
            GENMAT(IGEN,1:3,1:3)=DUM2(1:3,1:3)
            IF (DEBUG.OR.(IPRNT.GE.3)) PRINT*,'here E IGEN=',IGEN,' new GENMAT:'
            IF (DEBUG.OR.(IPRNT.GE.3)) PRINT '(9F12.4)',GENMAT(IGEN,1:3,1:3)
60          CONTINUE
         ENDIF
...
C
      ...
      IF (DEBUG.OR.(IPRNT.GE.3)) WRITE(*,770)
770   FORMAT(80('*'))
C
...
C
      IF (IHIGH.EQ.0) THEN
         WRITE(JNKSTR,'(A2,A1)') FPGRP(1:2),FPGRP(3:3)
      ...
      FPGRP=JNKSTR
      IF (IDEGEN.GT.0) THEN
         IF (DEBUG.OR.(IPRNT.GE.3)) WRITE(*,750) FPGRP
750      FORMAT(' The full molecular point group is ',A4,'.')
         WRITE(GPSTRING,750) FPGRP
      ENDIF
      IF (IDEGEN.EQ.0) THEN
         IF (DEBUG.OR.(IPRNT.GE.3)) WRITE(*,750) STRING(BPGRP,IHIGH)
         FPGRP= STRING(BPGRP,IHIGH)
         WRITE(GPSTRING,750) STRING(BPGRP,IHIGH)
      ENDIF
      IF (DEBUG.OR.(IPRNT.GE.3)) WRITE(*,790) BPGRP
790   FORMAT(' The largest Abelian subgroup of the full ',
     1       'molecular point group is ',A4,'.')
      IF (DEBUG.OR.(IPRNT.GE.3)) WRITE(*,760) TOLD, TOLE, TOLO
...
C
C SIMPLE SORTER FROM NUMERICAL RECIPES, MODIFIED TO MAKE JFS HAPPY.
C
        IMPLICIT NONE
        INTEGER N, J, I
     ...
     &           (Q1(3*(J1-1)+3)-Q2(3*(J2-1)+3))**2
            IF (DIST.LT.TOLDSQ) THEN
               ...
               IF (DIST.GT.DIST2) THEN
                  ...
                  JWORST=J1
               ENDIF
!              PRINT '(A,2I6,G20.10)','match J1,J2,DIST=',J1,J2,DIST
               EXIT innerloop
            ENDIF
         ENDDO innerloop
         IF (.NOT.PARTNER) THEN
C           PRINT '(A,I6)','no partner for atom ',J1
            DIST2=1.0D3
            RETURN
         ENDIF
      ENDDO 
...
!     PRINT '(A,2G20.10)','after full loop: DMIN,DIST2=',DMIN,DIST2

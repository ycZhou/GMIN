...
INTEGER :: J1, J2, NDUMMY, I, NORBIT, J, ISTART, LARGESIZE, NEWORB, J4, ISTAT
INTEGER :: J3, JP, NATOMSCORE, NORBITSCORE, NTOTAL, NMOVE, MOVEINDEX(NATOMS)
INTEGER :: NFLOAT, NLOST, LASTORBIT
...
INTEGER :: NQTOT, QDONE, BRUN, ITERATIONS, NCHOICE(NATOMS), NQTOTSAVE, IGEN, IGENSAVE, ORBSYM, NCORE, NORDER, NCHOOSE
INTEGER :: ICOMP, IPRNT, LASTLOST, PERM(NATOMS), NTOPSUM, NTOP, NTRIES, NSUB, NCORENEW, NEWCOREINDEX(NATOMS), NCORETMP, NMISS
INTEGER :: NMISSING(NATOMS), NSYMCALL, NMINREM, NPOSS
...
!  IF (NSYMCALL.GT.NORBIT-LASTORBIT+1) THEN
   IF (NSYMCALL.GT.1) THEN
      IF (LDEBUG) PRINT '(A,I5,A)','maximum calls to symmetry ',NORBIT-LASTORBIT,' exceeded for this minimum'
      RETURN 
   ENDIF
...
LTOLD=SYMTOL2
! PRINT '(A,3F15.5,L10)','in symmetry 1 EPREV(JP),POTEL,EBEST,CHANGEDE=',EPREV(JP),POTEL,EBEST,CHANGEDE

! XMASS=0.0D0
! YMASS=0.0D0
! ZMASS=0.0D0
! DO I=1,NATOMS
!    XMASS=XMASS+LCOORDS(3*(I-1)+1)
!    YMASS=YMASS+LCOORDS(3*(I-1)+2)
!    ZMASS=ZMASS+LCOORDS(3*(I-1)+3)
! ENDDO
! ORIGIN(1)=XMASS/NATOMS ! initial guess for origin
! ORIGIN(2)=YMASS/NATOMS
! ORIGIN(3)=ZMASS/NATOMS
! DO J=1,NATOMS
!    NINDEX(J)=J
...
cmloop: DO J1=1,200
   ...
   DO J=1,NATOMS
      CMDIST(J)=SQRT((LCOORDS(3*(J-1)+1)-ORIGIN(1))**2+ &
                     ...
                     (LCOORDS(3*(J-1)+3)-ORIGIN(3))**2)
   ENDDO
   IF (LDEBUG) PRINT '(A,I5,3G20.10)','cycle, origin:',J1,ORIGIN(1:3)
...
IF (LDEBUG) PRINT '(A,3F15.5)','initial centre of mass: ',ORIGIN(1:3)
...
IF (LDEBUG) PRINT '(A,3F15.5)','origin will be moved to centre of mass for atoms up to biggest gap: ',CMX(1),CMY(1),CMZ(1)
IF (LDEBUG) PRINT '(A,I6,A,F15.5,A)','number of atoms=',NDUMMY
ORBSIZE(1)=NDUMMY
IF (LDEBUG) PRINT '(12I5)', (NINDEX(J1),J1=1,ORBSIZE(1))
ORIGIN(1)=CMX(1)
ORIGIN(2)=CMY(1)
ORIGIN(3)=CMZ(1)
DO J=1,NATOMS
...
IF (LDEBUG) THEN
   ISTART=1
   OPEN(UNIT=1,FILE='orbits.xyz',STATUS='UNKNOWN')
   DO J1=1,NORBIT
      WRITE(1,'(I5)') SUM(ORBSIZE(1:J1))
      WRITE(1,'(A)') ' '
      DO J2=ISTART,ISTART+ORBSIZE(J1)-1
         WRITE(1,'(A2,2X,3G20.10)') 'LA',LCOORDS(3*(NINDEX(J2)-1)+1:3*(NINDEX(J2)-1)+3)
      ENDDO
      DO J2=1,ISTART-1
         WRITE(1,'(A2,2X,3G20.10)') 'LB',LCOORDS(3*(NINDEX(J2)-1)+1:3*(NINDEX(J2)-1)+3)
      ENDDO
...
IF (LARGESIZE.EQ.1) THEN
   IF (LDEBUG) THEN
      ...
      PRINT '(A,F15.2)','No nontrivial orbits - return from symmetry, time taken=',TIME-T0
   ENDIF
   RETURN
ENDIF
...
!     DUMMY=5.0D-3
      ...
      IF (LDEBUG) PRINT '(A,I5,A,I5,A,I4,A,A4)','number of orbits=',NORBITSCORE,' number of atoms=',NATOMSCORE, &
      ...
      ENDIF
   ELSE
      IF (LDEBUG) PRINT '(A,I5,A,I5)','number of orbits=',NORBITSCORE,' number of atoms=',NATOMSCORE
   ENDIF
   IF (NATOMSCORE.GT.20) EXIT symcore ! looks like there is simply no symmetry
ENDDO symcore

IF (IGENSAVE.LE.0) THEN
   IF (LDEBUG) PRINT '(A)','no symmetry detected'
   RETURN
ELSE
   IF (NCORE.EQ.NATOMS) THEN
      RETURN
   ENDIF
   PRINT '(A,A,A,I4)','symmetry analysis for point group ',TRIM(ADJUSTL(POINTGROUP)),' number of generators=',IGENSAVE
!  PRINT '(A,I5,A,I5,A)','this group was detected for the first ',ORBSYM,' orbits containing ',NCORE,' atoms'
ENDIF
...
ENDDO
! 
! Refine the IGENSAVE symmetry operations for the NCORE atoms in
...
IF (INFO.NE.0) THEN
   PRINT '(A,I6)','ERROR - INFO after DGETRI in symmetry=',INFO
   PRINT '(A)','CORECOORDS:'
   PRINT '(3F20.10)',CORECOORDS(1:3*NCORE)
   PRINT '(A)','LHESS'
   PRINT '(3F20.10)',LHESS
   STOP
ENDIF 
...
!  SYMOP(1+J1,1:3,1:3)=GENMATSAVE(J1,1:3,1:3)
   ...
   IF (LDEBUG) PRINT '(A,I5,9F10.4)','NORDER,initial MAT: ',NORDER,GENMATSAVE(J1,1:3,1:3)
   CALL MATMULV(NEWQ,CORECOORDS,SYMOP1,NCORE,3,3)
!  CALL MINPERM(NCORE,NEWQ,CORECOORDS,BOXLX,BOXLY,BOXLZ,PERIODIC,PERM,DUMMY,DIST2,WORSTRAD)
   CALL TESTSYMOP(NCORE,NEWQ,CORECOORDS,PERM,LTOLD,DIST2,WORSTRAD)
   IF (DIST2.GT.LTOLD) THEN
      PRINT '(A,2G20.10)','ERROR - genloop in symmetry, DIST2,LTOLD=',DIST2,LTOLD
      cycle genloop
   ENDIF
!  CALL BIPARTITE(NCORE,NEWQ,CORECOORDS,PERM,DUMMY,DIST2,WORSTRAD)
!  PRINT '(A,6F15.5)','dists: ',DUMMY,DIST2,WORSTRAD,BDUMMY,BDIST2,BWORSTRAD
 ...
 &                     (CORECOORDS(3*(PERM(J2)-1)+3)-NEWQ(3*(J2-1)+3))**2
      ENDDO
      PRINT '(A,I5,G20.10)','operation,     error=',J1,SERROR
...
!
   DO J2=1,NORDER
      ...
      ENDDO newoploop
      IF (.NOT.NEWOP) THEN
         IF (LDEBUG) PRINT '(2(A,I6))','generator ',J1,' appears to be the same as ',J2
         CYCLE genloop
      ENDIF
   ENDDO
!
...
!
   ...
   IF (LDEBUG) THEN
      CALL MATMULV(NEWQ,CORECOORDS,SYMOP1,NCORE,3,3)
!     CALL MINPERM(NCORE,NEWQ,CORECOORDS,BOXLX,BOXLY,BOXLZ,PERIODIC,PERM2,DUMMY,DIST2,WORSTRAD)
      CALL TESTSYMOP(NCORE,NEWQ,CORECOORDS,PERM2,LTOLD,DIST2,WORSTRAD)
!     CALL BIPARTITE(NCORE,NEWQ,CORECOORDS,PERM2,DUMMY,DIST2,WORSTRAD)
      SERROR2=0.0D0
      DO J2=1,NCORE
 ...
 &                       (CORECOORDS(3*(PERM2(J2)-1)+3)-NEWQ(3*(J2-1)+3))**2
      ENDDO
      PRINT '(A,I5,G20.10)','operation, new error=',J1,SERROR2
      IF (SERROR2.GT.SERROR) PRINT '(A)',' WARNING - error has increased - this should never happen'
   ENDIF
   NORDER=NORDER+1
   SYMOP(NORDER,1:3,1:3)=SYMOP1(1:3,1:3)
   GENPERM(NORDER,1:NCORE)=PERM(1:NCORE)
   OPPERM(NORDER,1:NCORE)=PERM(1:NCORE)
   IF (LDEBUG) PRINT '(A,I5,9F10.4)','NORDER,MAT: ',NORDER,SYMOP(NORDER,1:3,1:3)
   IF (LDEBUG) PRINT '(20I6)',PERM(1:NCORE)
ENDDO genloop
!
...
!         ENDDO
         ...
         IF (NORDER.GT.120) THEN
            PRINT '(A)','WARNING more than 120 operations case A!'
!           STOP
            NORDER=120
            EXIT makegroup
         ELSE
...
CALL MYCPU_TIME(TIME)
PRINT '(A,I5,A,F15.2)','using point group of order ',NORDER,' constructed from generators, time taken=',TIME-T0
IF (LDEBUG) THEN
   DO J1=1,NORDER
      PRINT '(A,I5,9F10.4)','J1,SYMOP: ',J1,SYMOP(J1,1:3,1:3)
   ENDDO
ENDIF
! test to check that we really have symmetry operations!
IF (LDEBUG) THEN
   OPEN(UNIT=1,FILE='testsym2.xyz',STATUS='UNKNOWN')
   DO J1=1,NORDER
      SYMOP1(1:3,1:3)=SYMOP(J1,1:3,1:3)
      CALL MATMULV(NEWQ,CORECOORDS,SYMOP1,NCORE,3,3)
!     CALL MINPERM(NCORE,NEWQ,CORECOORDS,BOXLX,BOXLY,BOXLZ,PERIODIC,PERM,DUMMY,DIST2,WORSTRAD)
...
!     WRITE(1,'(I5)') NATOMS
      WRITE(1,'(A,I5,2(A,F15.5))') 'operation ',J1,' dist=',DUMMY,' dist2=',DIST2
      WRITE(1,'(A2,2X,3G20.10)') ('LA',NEWQ(3*(J2-1)+1),NEWQ(3*(J2-1)+2),NEWQ(3*(J2-1)+3),J2=1,NCORE)
   ENDDO
   CLOSE(1)
ENDIF
...
symother: DO J1=NORBITSCORE+1,NORBIT
   ...
   IF (LDEBUG) PRINT '(4(A,I5))','testing symmetry for orbit ',J1,' of ',ORBSIZE(J1),' atoms, running total=',NTOTAL, &
                            ' symmetry elements remaining=',NSYMOP

   DO J2=ISTART,ISTART+ORBSIZE(J1)-1
      ORBCOORDS(3*(J2-ISTART)+1:3*(J2-ISTART)+3)=LCOORDS(3*(NINDEX(J2)-1)+1:3*(NINDEX(J2)-1)+3)
   ENDDO
   IF (LDEBUG)  OPEN(UNIT=1,FILE='temp.xyz',STATUS='UNKNOWN')
   IF (LDEBUG)  WRITE(1,'(I5)') NCORE+ORBSIZE(J1)
   IF (LDEBUG)  WRITE(1,*) ' '
   IF (LDEBUG)  WRITE(1,'(A2,2X,3G20.10)') ('LA',CORECOORDS(3*(J2-1)+1:3*(J2-1)+3),J2=1,NCORE)
   IF (LDEBUG)  WRITE(1,'(A2,2X,3G20.10)') ('LB',ORBCOORDS(3*(J2-1)+1:3*(J2-1)+3),J2=1,ORBSIZE(J1))
   IF (LDEBUG)  CLOSE(1)
   ISTART=ISTART+ORBSIZE(J1)
   IF (LDEBUG)  OPEN(UNIT=2,FILE='temp2.xyz',STATUS='UNKNOWN')
...
!        IF (DIST2.GT.SYMTOL4) THEN
            SYMLOST(J3)=.TRUE.
            LASTORBIT=J1-1
            IF (LDEBUG.AND.(.NOT.DOMISSING)) PRINT '(A,I5,A,2F15.5)','symmetry element ',J3,' lost, DUMMY,DIST2=',DUMMY,DIST2
            NSYMOP=NSYMOP-1
            NLOST=NLOST+1
...
!
            ...
            CHANGE=.TRUE.
            IF (LDEBUG) THEN
               WRITE(2,'(I5)') 2*ORBSIZE(J1)+SUM(ORBSIZE(1:J1-1))
               WRITE(2,'(A,I5,2F15.5)') 'operation,DUMMY,dist2=',J3,DUMMY,DIST2
               WRITE(2,'(A2,2X,3G20.10)') ('LA',TRANSCOORDS(3*(J2-1)+1:3*(J2-1)+3),J2=1,ORBSIZE(J1))
               WRITE(2,'(A2,2X,3G20.10)') ('LB',ORBCOORDS(3*(J2-1)+1:3*(J2-1)+3),J2=1,ORBSIZE(J1))
               DO J2=1,ISTART-ORBSIZE(J1)-1
                  WRITE(2,'(A2,2X,3G20.10)') 'LB',LCOORDS(3*(NINDEX(J2)-1)+1:3*(NINDEX(J2)-1)+3)
               ENDDO
            ENDIF
            IF (DOMISSING) THEN
               transloop: DO J2=1,ORBSIZE(J1) ! cycle over transformed coordinates
                  ...
                  IF (NMISSING(J1).EQ.120) THEN
                     PRINT '(A)','warning - too many missing sites'
                     EXIT transloop
                  ENDIF
                  NMISSING(J1)=NMISSING(J1)+1
                  QMISSING(J1,3*(NMISSING(J1)-1)+1:3*(NMISSING(J1)-1)+3)=TRANSCOORDS(3*(J2-1)+1:3*(J2-1)+3)
!                 IF (LDEBUG) PRINT '(A,I5,A,I5,A,3F15.5)','orbit ',J1,' missing site ',NMISSING(J1), &
!                             ' coords: ',TRANSCOORDS(3*(J2-1)+1:3*(J2-1)+3)
               ENDDO transloop
               IF (LDEBUG) PRINT '(A,I5,A,2F15.5,A,I5)','symmetry element ',J3,' lost, dists=',DUMMY,DIST2, &
                                                       ' missing sites=',NMISSING(J1)
            ENDIF
         ELSE
            IF (LDEBUG) PRINT '(A,I5,A)','symmetry element ',J3,' retained'
         ENDIF
      ENDIF
   ENDDO
   IF (LDEBUG)  CLOSE(2)
   ...
   IF (NSYMOP.EQ.1) THEN
      IF (LDEBUG) PRINT '(A,I5)','only the identity element remains for orbit ',J1
      EXIT symother
   ENDIF
ENDDO symother
...
IF (LASTLOST.EQ.0) THEN
   PRINT '(2(A,I5))','symmetry of core is consistent with the overall symmetry'
   RETURN
ELSE
   PRINT '(2(A,I5))','number of symmetry elements lost in last change for orbit ',LASTORBIT+1,' was ',LASTLOST
ENDIF

IF (LDEBUG) THEN
   ISTART=1
   OPEN(UNIT=1,FILE='orbits2.xyz',STATUS='UNKNOWN')
   DO J1=1,NORBIT
      WRITE(1,'(I5)') SUM(ORBSIZE(1:J1))+NMISSING(J1)
      WRITE(1,'(A)') ' '
      DO J2=ISTART,ISTART+ORBSIZE(J1)-1
         WRITE(1,'(A2,2X,3G20.10)') 'LA',LCOORDS(3*(NINDEX(J2)-1)+1:3*(NINDEX(J2)-1)+3)
      ENDDO
      ...
      ENDDO
      DO J2=1,NMISSING(J1)
         WRITE(1,'(A2,2X,3G20.10)') 'LC',QMISSING(J1,3*(J2-1)+1:3*(J2-1)+3)
      ENDDO
      ISTART=ISTART+ORBSIZE(J1)
   ENDDO
   CLOSE(1)
         ...
         missloop: DO J3=1,NMISSING(J2)
            DO J4=NMISS+1,NATOMS
               ...
               IF (DUMMY.LT.SYMTOL5) THEN
                  PRINT '(3(A,I5))','missing site ',J3,' from orbit ',J2,' is too close to atom ',J4
                  CYCLE missloop
               ENDIF
            ENDDO
            NMISS=NMISS+1
...
!     IF (NMISS.GT.12) EXIT missopt
      ...
      IF (LDEBUG) PRINT '(A,I5,A)','filling ',NMISS,' missing sites'
      
      NQTOT=NQTOT+1
      NQ(JP)=NQ(JP)+1
      CALL QUENCH(.FALSE.,JP,ITERATIONS,TIME,BRUN,QDONE,SCREENC)
      WRITE(*,'(A,I10,A,F20.10,A,I5,A,G12.5,30X,A,F11.1)') 'Qu ',NQ(JP),' E=',POTEL,' steps=',ITERATIONS,' RMS=',RMS,' t=',TIME
      CALL FLUSH(6,ISTAT)
...
!  NCHOOSE=NORBIT-NSYMCALL+2 
   ...
   PRINT '(A,I5)','changing CLOSEOP to the closest lost operation for orbit ',NCHOOSE
...
!  ENDDO
   IF (DPRAND().GT.0.0D0) THEN ! add remaining symmetry elements to the LASTLOST set to complete that subgroup
      DO J3=2,NORDER ! cycle over remaining point group operations - the identity is not needed
         ...
         ENDIF
      ENDDO
      PRINT '(3(A,I5))','after adding the remaining ',NSYMOP-1,' non-identity elements there are ',LASTLOST,' in total'
   ELSE ! or, with 0% probability, just use the full symmetry. In this case we will only permute single weakly bound atoms.
      ...
      DO J3=2,NORDER ! cycle over remaining point group operations - the identity is not needed
         ...
         ENDIF
      ENDDO
      PRINT '(3(A,I5))','using the remaining ',NSYMOP-1,' non-identity elements'
   ENDIF
!  OPEN(UNIT=1,FILE='testsym3.xyz',STATUS='UNKNOWN')
!  WRITE(1,'(I5)') NATOMS
!  WRITE(1,'(A,I5)') 'operation ',1
!  WRITE(1,'(A2,2X,3G20.10)') ('LA',LCOORDS(3*(J2-1)+1),LCOORDS(3*(J2-1)+2),LCOORDS(3*(J2-1)+3),J2=1,NATOMS)
!  DO J3=1,NSYMREM
!     SYMOP1(1:3,1:3)=SYMREM(J3,1:3,1:3)
!     CALL MATMULV(NEWQ,LCOORDS,SYMOP1,NATOMS,3,3)
!     WRITE(1,'(I5)') NATOMS
!     WRITE(1,'(A,I5)') 'operation ',J3+1
!     WRITE(1,'(A2,2X,3G20.10)') ('LA',NEWQ(3*(J2-1)+1),NEWQ(3*(J2-1)+2),NEWQ(3*(J2-1)+3),J2=1,NATOMS)
!  ENDDO
!  CLOSE(1)
!
...
! Move coordinates into CORE and OTHER vectors. Define NCORE, the number of core atoms.
! CORECOORDS and NCORE are expanded to include all the atoms in orbits that are invariant
...
DO J1=LASTORBIT+1,NORBIT
   DO J2=ISTART,ISTART+ORBSIZE(J1)-1
      OTHERCOORDS(3*(J2-NCORE-1)+1:3*(J2-NCORE-1)+3)=LCOORDS(3*(NINDEX(J2)-1)+1:3*(NINDEX(J2)-1)+3)
      OTHERVT(J2-NCORE)=VT(NINDEX(J2))
   ENDDO
   ISTART=ISTART+ORBSIZE(J1)
ENDDO
...
! The idea is to avoid propagation of numerical errors.
            ...
            NSUB=NSUB+1
            IF (NSUB.GT.120) THEN
               PRINT '(A)','WARNING more than 120 operations case B!'
               NSUB=120
!              STOP
               EXIT makesub
            ELSE
   ...
   PRINT '(A,I5,A)','using subgroup of order ',NSUB,' constructed from lost operation'
   IF (LDEBUG) THEN
      DO J1=1,NSUB
         PRINT '(A,I5,9F10.4)','J1,SUBOP: ',J1,SUBOP(J1,1:3,1:3)
      ENDDO
   ENDIF
...
!  PRINT '(A,I5,3G20.10)','J1,COREVT,VMIN,ASTEP(JP)*VMIN=',J1,COREVT(J1),VMIN,ASTEP(JP)*VMIN
   ...
   IF (LDUMMY) THEN
      ...
      IF (DIST2.LE.SYMTOL4) THEN ! atom lies on the symmetry element 
         ...
         NMOVE=NMOVE+1
         MOVEINDEX(NMOVE)=J1
         IF (LDEBUG) PRINT '(A,I5,A,2I5,A,F15.5)','weakly bound invariant atom ',J1, &
                          ' added to floater list, NCORE,NFLOAT=',NCORE-NMOVE,NFLOAT, &
...
NEWORB=0
NCORENEW=0
MOVEDTOCORE(1:NATOMS-NCORE)=.FALSE.
otherloop: DO J1=1,NATOMS-NCORE
...
! compare with members of previous orbits
   ...
   applyops: DO J2=1,LASTLOST
      ...
      ENDDO 
      DO J3=1,NCORE+NCORENEW ! check overlaps with core atoms
...
!                                                    ' DUMMY=',DUMMY
                  ...
                  NCORETMP=NCORETMP+1
                  CORECOORDS(3*(NCORE+NCORENEW+NCORETMP-1)+1:3*(NCORE+NCORENEW+NCORETMP-1)+3)= &
                                OTHERCOORDS(3*(J3-1)+1:3*(J3-1)+3)
                  COREVT(NCORE+NCORENEW+NCORETMP)=OTHERVT(J3)
...
!                 PRINT '(A,I5,A,I5,A,G20.10)','new orbit atom ',J2,' matches floater ',J3,' DUMMY=',DUMMY
   ...
   IF (MATCHED.AND.(NCORETMP.GT.1)) THEN ! leave single atoms as floaters, otherwise move to core
      ...
      IF (LDEBUG) PRINT '(A,I5,A,120I5)','moving ',NCORETMP,' atoms to the core: ',NEWCOREINDEX(1:NCORETMP)
      IF (LDEBUG) PRINT '(A,I5)','number of core atoms=',NCORE+NCORENEW
      DO J2=1,NCORETMP
...
! Otherwise, store the new orbit.
   ...
   IF (LDEBUG) PRINT '(3(A,I6))','new orbit ',NEWORB,' of dimension ',NEWORBSIZE(NEWORB), &
...
NCORE=NCORE+NCORENEW

IF (LDEBUG) THEN
   OPEN(1,FILE='coreplusother.xyz',STATUS='UNKNOWN')
   WRITE(1,*) NATOMS
   WRITE(1,*) ' '
   WRITE(1,'(A2,3X,3F20.10)') ('LA',CORECOORDS(3*(J2-1)+1:3*(J2-1)+3),J2=1,NCORE)
   NDUMMY=0
   DO J1=1,NATOMS
      IF (.NOT.MOVEDTOCORE(J1)) THEN
         WRITE(1,'(A2,3X,3F20.10)') 'LB',OTHERCOORDS(3*(J1-1)+1:3*(J1-1)+3)
         NDUMMY=NDUMMY+1
         IF (NDUMMY.EQ.NATOMS-NCORE) EXIT
      ENDIF
   ENDDO
   CLOSE(1)

   OPEN(UNIT=1,FILE='neworbits.xyz',STATUS='UNKNOWN')
   DO J1=1,NEWORB
      WRITE(1,'(I6)') SUM(NEWORBSIZE(1:J1))+NCORE
!     WRITE(1,'(I6)') NEWORBSIZE(J1)
      WRITE(1,'(A)') ' '
      WRITE(1,'(A2,3X,3F20.10)') ('LA',CORECOORDS(3*(J2-1)+1:3*(J2-1)+3),J2=1,NCORE)
      DO J3=1,J1-1
         WRITE(1,'(A2,3X,3F20.10)') ('LC',NEWORBCOORDS(J3,3*(J2-1)+1:3*(J2-1)+3),J2=1,NEWORBSIZE(J3))
      ENDDO
!     WRITE(1,'(A2,3X,3F20.10)') (('LB',NEWORBCOORDS(J1,3*(J2-1)+1:3*(J2-1)+3),J2=1,NEWORBSIZE(J1)),J1=1,1)
      WRITE(1,'(A2,3X,3F20.10)') ('LB',NEWORBCOORDS(J1,3*(J2-1)+1:3*(J2-1)+3),J2=1,NEWORBSIZE(J1))
   ENDDO
   CLOSE(1)

   OPEN(UNIT=1,FILE='newpluscore.xyz',STATUS='UNKNOWN')
   WRITE(1,'(I6)') SUM(NEWORBSIZE(1:NEWORB))+NCORE
   WRITE(1,'(A)') ' '
   WRITE(1,'(A2,3X,3F20.10)') ('LA',CORECOORDS(3*(J2-1)+1:3*(J2-1)+3),J2=1,NCORE)
   WRITE(1,'(A2,3X,3F20.10)') (('LB',NEWORBCOORDS(J1,3*(J2-1)+1:3*(J2-1)+3),J2=1,NEWORBSIZE(J1)),J1=1,NEWORB)
   CLOSE(1)
ENDIF
IF (NCORE.EQ.NATOMS) RETURN
...
!
! Now fill in the NEWORB orbits with the NATOMS-NCORE=NFLOAT candidates. 
...
IF (.TRUE.) THEN
   ...
   perms2: DO NTRIES=1,MIN(NSYMQMAX,NPOSS) ! NPOSS is the number of permutations saved
      ...
      IF (NDUMMY.NE.NFLOAT) THEN
         PRINT '(A,2I5,A)','ERROR - NDUMMY,NFLOAT=',NDUMMY,NFLOAT,' in symmetry'
         STOP
      ENDIF
      IF (LDEBUG) PRINT '(A)','NCHOICE:'
      IF (LDEBUG) PRINT '(60I2)',NCHOICE(1:NEWORB)
      IF (LDEBUG) PRINT '(A,2I5)','NDUMMY,NFLOAT=',NDUMMY,NFLOAT
      COORDS(1:3*NCORE,JP)=CORECOORDS(1:3*NCORE)
      NDUMMY=NCORE
      DO J1=1,NEWORB
...
ELSE ! old enumeration
   ...
   !     PRINT*,'NTOP,NTOPSUM,NFLOAT=',NTOP,NTOPSUM,NFLOAT
      ...
      IF (NDUMMY.NE.NATOMS) THEN
         PRINT '(A,2I5)','ERROR - NDUMMY,NATOMS=',NDUMMY,NATOMS
         STOP
      ENDIF
      NQTOT=NQTOT+1
      NQ(JP)=NQ(JP)+1
...
IF (PTEST) WRITE(*,'(A,2F20.10,A,I6,A)') ' func and RMS force=',ENERGY,RMS,' after ',ITDONE,' LBFGS steps'
...
!        IF (PTEST) PRINT*,'Search direction has positive projection onto gradient - reset'
   DO I=1,N
      W(ISPT+POINT*N+I)= -W(I)
   ENDDO
!        ITER=0
...
!        IF (STP*SLENGTH.LT.1.0D-10) THEN
   IF (NDECREASE.GT.5) THEN
      NFAIL=NFAIL+1
      PRINT*,' in smylbfgs LBFGS step cannot find a lower distance, NFAIL=',NFAIL
   ...
   STP=STP/10.0D0
   IF (PTEST) &
     &    WRITE(*,'(A,F19.10,A,F16.10,A,F15.8)') ' func increased from ',ENERGY,' to ',ENEW, &
...
DO J1=1,N
   G(J1)=GSAVE(J1)
ENDDO
IF (PTEST) WRITE(*,'(A,2F20.10,A,I6,A,G15.5)') ' func and RMS force=',ENERGY,RMS,' after ',ITDONE, &
...
ISTART=1
DO J=1,NORBIT
   IF (LDEBUG) PRINT '(A,I6,A,I6,A,F15.5)','orbit ',J,' size ',ORBSIZE(J),' distance ',ORBDIST(J)
   IF (LDEBUG) PRINT '(A,I6)','members of orbit ',J
   IF (LDEBUG) PRINT '(12I5)', (NINDEX(J1),J1=ISTART,ISTART+ORBSIZE(J)-1)
   IF (LDEBUG) PRINT '(A)','distances:'
   IF (LDEBUG) PRINT '(6G20.10)', (CMDIST(J1),J1=ISTART,ISTART+ORBSIZE(J)-1)
   ...
   ISTART=ISTART+ORBSIZE(J)
ENDDO
IF (LDEBUG) PRINT '(A,I6,A,I6)', 'largest orbit has ',LARGESIZE,' members starting from sorted position ',LARGES
